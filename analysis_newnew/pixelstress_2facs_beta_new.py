# step2_beta_plots_and_clusters_masks_topos_plus_roi_bins.py
#
# Theta-band version of your step2 script:
# - Uses TF beta-band betas: index.tf_beta_betas_path
# - Uses baseline-corrected sequence TFR (4D) for ROI bin plot: index.tfr_epochs_path
# - Heatmaps + cluster contours + topomaps work on beta-band betas (Evoked)
# - ROI/bin plot uses beta-band power (collapsed freq) averaged over CLUSTER CHANNELS
#
# Assumptions:
# - subject_index.csv contains:
#     tf_beta_betas_path, tfr_epochs_path, group, id
# - tfr_epochs_path points to power_seq_bl saved via .save(... .h5)
# - power_seq_bl.metadata contains column 'f' and 'group'
# - power_seq_bl.freqs contains the original freqs (4..30 step 2), so beta mask works.
...
# beta band definition (must match your regression step)
BETA = (16.0, 30.0)
...
# =====================================================================
# 1) Load BETA betas for all subjects
# =====================================================================
betas_all = []
for row in index.itertuples(index=False):
    betas = load_betas_evoked(row.tf_beta_betas_path)  # <-- beta betas
...
print(f"[beta beta_f] significant clusters: {np.sum(pvals_f < beta)} (beta={beta})")
...
print(f"[beta beta_f2] significant clusters: {np.sum(pvals_f2 < beta)} (beta={beta})")
...
if len(betas_exp) < 5 or len(betas_ctl) < 5:
    print("[beta Intercept EXP vs CTL] Too few subjects per group; skipping test.")
else:
    ...
    print(
        f"[beta Intercept EXP vs CTL] significant clusters: {np.sum(pvals_int < beta)} (beta={beta})"
    )
...
times_f2g = None
if len(betas_exp) < 5 or len(betas_ctl) < 5:
    print("[beta beta_f2 EXP vs CTL] Too few subjects per group; skipping test.")
else:
    ...
    print(
        f"[beta beta_f2 EXP vs CTL] significant clusters: {np.sum(pvals_f2g < beta)} (beta={beta})"
    )

# =====================================================================
# 3) Heatmaps + topomaps (beta betas)
# =====================================================================
...
plot_beta_heatmap_with_clusters(
    ...
    times,
    ch_names_plot,
    title=f"BETA β_f: mean across subjects (n={len(evs_f)})",
    vlim=vmax_f,
    clusters=clusters_f,
    pvals=pvals_f,
    beta=beta,
...
plot_beta_heatmap_with_clusters(
    ...
    times,
    ch_names_plot,
    title=f"BETA β_f²: mean across subjects (n={len(evs_f2)})",
    vlim=vmax_f2,
...
if len(betas_exp) >= 2 and len(betas_ctl) >= 2:
    ...
    plot_beta_heatmap_with_clusters(
        ...
        times_int,
        ch_names_plot_int,
        title="BETA Intercept group difference (EXP − CTL)",
        vlim=vmax_int,
    ...
    print("Not enough subjects per group for beta intercept difference plot.")
...
if len(betas_exp) >= 2 and len(betas_ctl) >= 2:
    ...
    plot_beta_heatmap_with_clusters(
        ...
        times_f2g,
        ch_names_plot_f2g,
        title="BETA f² × group interaction proxy (β_f² EXP − β_f² CTL)",
        vlim=vmax_f2g,
    ...
    print("Not enough subjects per group for beta f2×group plot.")
...
good_f = np.where(pvals_f < beta)[0]
for ci in good_f:
    plot_cluster_topomap(
        topo_template, mean_map_f, times, clusters_f[ci],
        title_prefix=f"Topomap: BETA β_f (cluster {ci})",
        mask_sensors=True,
    )

good_f2 = np.where(pvals_f2 < beta)[0]
for ci in good_f2:
    plot_cluster_topomap(
...
if clusters_int is not None and pvals_int is not None and diff_map_int is not None:
    good_int = np.where(pvals_int < beta)[0]
    for ci in good_int:
        plot_cluster_topomap(
            topo_template, diff_map_int, times_int, clusters_int[ci],
            title_prefix=f"Topomap: BETA Intercept EXP−CTL (cluster {ci})",
            mask_sensors=True,
        )

if clusters_f2g is not None and pvals_f2g is not None and diff_map_f2g is not None:
    good_f2g = np.where(pvals_f2g < beta)[0]
    for ci in good_f2g:
        plot_cluster_topomap(
            topo_template, diff_map_f2g, times_f2g, clusters_f2g[ci],
            title_prefix=f"Topomap: BETA β_f² EXP−CTL (cluster {ci})",
            mask_sensors=True,
        )

# =====================================================================
# 4) BETA ROI bin plot over CLUSTER CHANNELS from beta f2 one-sample cluster
...
roi_label = "beta_beta_f2"
n_bins = 7
time_window = None  # e.g. (-0.2, 0.8) or None

# --- pick significant beta f2 cluster (from betas) ---
good = np.where(pvals_f2 < beta)[0]
if len(good) == 0:
    raise RuntimeError(f"No significant clusters for {roi_label} at beta={beta}.")
if len(good) > 1:
    print(f"Warning: {len(good)} significant clusters for {roi_label}; using smallest p.")
ci = good[np.argmin(pvals_f2[good])]

mask_tc = clusters_f2[ci]          # (n_times, n_ch) from beta betas
...
print(f"ROI: BETA f2 cluster {ci} p={pvals_f2[ci]:.4f}, n_channels={len(roi_ch_names)}")
...
# -------------------------------------------------
# 2) Per-subject bin means over ROI channels, using BETA power
# -------------------------------------------------
sub_bin_rows = []
times_roi = None
...
for r in index.itertuples(index=False):
    ...
    # pick beta freqs and collapse freqs -> (n_seq, n_ch, n_time)
    fmask = (tfr.freqs >= BETA[0]) & (tfr.freqs <= BETA[1])
    if not np.any(fmask):
        raise RuntimeError(f"No beta freqs found in TFR for {r.tfr_epochs_path}")

    # data: (n_seq, n_ch, n_freq, n_time) -> mean over freq -> (n_seq, n_ch, n_time)
    data_beta = tfr.data[:, :, fmask, :].mean(axis=2)

    # ROI average across channels -> (n_seq, n_time)
    y = data_beta.mean(axis=1)
    times_roi = tfr.times
...
if len(df_roi) == 0:
    raise RuntimeError("No ROI beta data computed. Check ROI channels and file paths.")
...
for ax, g in zip(axes, groups):
    for b in bins:
        ...
        ax.fill_between(times_roi, m - s, m + s, color=color, beta=0.25)

    ax.axhline(0, linestyle="--", linewidth=1)
    ax.axvline(0, linestyle="--", linewidth=1)
    ax.set_title(f"BETA power ROI = f² cluster channels (p={pvals_f2[ci]:.3f}) — {g}")
