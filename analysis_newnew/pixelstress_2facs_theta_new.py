# step2_theta_plots_and_clusters_masks_topos_plus_roi_bins.py
#
# Theta-band version of your step2 script:
# - Uses TF theta-band betas: index.tf_theta_betas_path
# - Uses baseline-corrected sequence TFR (4D) for ROI bin plot: index.tfr_epochs_path
# - Heatmaps + cluster contours + topomaps work on theta-band betas (Evoked)
# - ROI/bin plot uses theta-band power (collapsed freq) averaged over CLUSTER CHANNELS
#
# Assumptions:
# - subject_index.csv contains:
#     tf_theta_betas_path, tfr_epochs_path, group, id
# - tfr_epochs_path points to power_seq_bl saved via .save(... .h5)
# - power_seq_bl.metadata contains column 'f' and 'group'
# - power_seq_bl.freqs contains the original freqs (4..30 step 2), so theta mask works.
...
# theta band definition (must match your regression step)
THETA = (4.0, 7.0)
...
# =====================================================================
# 1) Load THETA betas for all subjects
# =====================================================================
betas_all = []
for row in index.itertuples(index=False):
    betas = load_betas_evoked(row.tf_theta_betas_path)  # <-- theta betas
...
print(f"[theta beta_f] significant clusters: {np.sum(pvals_f < alpha)} (alpha={alpha})")
...
print(f"[theta beta_f2] significant clusters: {np.sum(pvals_f2 < alpha)} (alpha={alpha})")
...
if len(betas_exp) < 5 or len(betas_ctl) < 5:
    print("[theta Intercept EXP vs CTL] Too few subjects per group; skipping test.")
else:
    ...
    print(
        f"[theta Intercept EXP vs CTL] significant clusters: {np.sum(pvals_int < alpha)} (alpha={alpha})"
    )
...
times_f2g = None
if len(betas_exp) < 5 or len(betas_ctl) < 5:
    print("[theta beta_f2 EXP vs CTL] Too few subjects per group; skipping test.")
else:
    ...
    print(
        f"[theta beta_f2 EXP vs CTL] significant clusters: {np.sum(pvals_f2g < alpha)} (alpha={alpha})"
    )

# =====================================================================
# 3) Heatmaps + topomaps (theta betas)
# =====================================================================
...
plot_beta_heatmap_with_clusters(
    ...
    times,
    ch_names_plot,
    title=f"THETA β_f: mean across subjects (n={len(evs_f)})",
    vlim=vmax_f,
    clusters=clusters_f,
    pvals=pvals_f,
    alpha=alpha,
...
plot_beta_heatmap_with_clusters(
    ...
    times,
    ch_names_plot,
    title=f"THETA β_f²: mean across subjects (n={len(evs_f2)})",
    vlim=vmax_f2,
...
if len(betas_exp) >= 2 and len(betas_ctl) >= 2:
    ...
    plot_beta_heatmap_with_clusters(
        ...
        times_int,
        ch_names_plot_int,
        title="THETA Intercept group difference (EXP − CTL)",
        vlim=vmax_int,
    ...
    print("Not enough subjects per group for theta intercept difference plot.")
...
if len(betas_exp) >= 2 and len(betas_ctl) >= 2:
    ...
    plot_beta_heatmap_with_clusters(
        ...
        times_f2g,
        ch_names_plot_f2g,
        title="THETA f² × group interaction proxy (β_f² EXP − β_f² CTL)",
        vlim=vmax_f2g,
    ...
    print("Not enough subjects per group for theta f2×group plot.")
...
for ci in good_f:
    plot_cluster_topomap(
        ...
        title_prefix=f"Topomap: THETA β_f (cluster {ci})",
        mask_sensors=True,
    )
...
if clusters_int is not None and pvals_int is not None and diff_map_int is not None:
    ...
    for ci in good_int:
        plot_cluster_topomap(
            ...
            title_prefix=f"Topomap: THETA Intercept EXP−CTL (cluster {ci})",
            mask_sensors=True,
        )
...
if clusters_f2g is not None and pvals_f2g is not None and diff_map_f2g is not None:
    ...
    for ci in good_f2g:
        plot_cluster_topomap(
            ...
            title_prefix=f"Topomap: THETA β_f² EXP−CTL (cluster {ci})",
            mask_sensors=True,
        )

# =====================================================================
# 4) THETA ROI bin plot over CLUSTER CHANNELS from theta f2 one-sample cluster
...
roi_label = "theta_beta_f2"
n_bins = 7
time_window = None  # e.g. (-0.2, 0.8) or None

# --- pick significant theta f2 cluster (from betas) ---
...
mask_tc = clusters_f2[ci]          # (n_times, n_ch) from theta betas
...
print(f"ROI: THETA f2 cluster {ci} p={pvals_f2[ci]:.4f}, n_channels={len(roi_ch_names)}")
...
# -------------------------------------------------
# 2) Per-subject bin means over ROI channels, using THETA power
# -------------------------------------------------
sub_bin_rows = []
times_roi = None
...
for r in index.itertuples(index=False):
    ...
    # pick theta freqs and collapse freqs -> (n_seq, n_ch, n_time)
    fmask = (tfr.freqs >= THETA[0]) & (tfr.freqs <= THETA[1])
    if not np.any(fmask):
        raise RuntimeError(f"No theta freqs found in TFR for {r.tfr_epochs_path}")

    # data: (n_seq, n_ch, n_freq, n_time) -> mean over freq -> (n_seq, n_ch, n_time)
    data_theta = tfr.data[:, :, fmask, :].mean(axis=2)

    # ROI average across channels -> (n_seq, n_time)
    y = data_theta.mean(axis=1)
    times_roi = tfr.times
...
if len(df_roi) == 0:
    raise RuntimeError("No ROI theta data computed. Check ROI channels and file paths.")
...
for ax, g in zip(axes, groups):
    ...
    ax.set_title(f"THETA power ROI = f² cluster channels (p={pvals_f2[ci]:.3f}) — {g}")
