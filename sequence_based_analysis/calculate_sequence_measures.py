...
def prepare_epochs_for_psd(epochs: mne.Epochs) -> mne.Epochs:
    """
    Optionally downsample before PSD estimation.
    """
...
def run_pipeline(
    ...
    """
    Run the full sequence-extraction pipeline for CAR-referenced data.
    """
...
def process_subject(dataset: Path):
    ...
    # Loaded data are assumed to already be CAR-referenced.
    df_seq, seq_psd, seq_index, freqs = run_pipeline(
        epochs=epochs,
        df_trials=df_trials,
        subj_id=subj_id,
    )
