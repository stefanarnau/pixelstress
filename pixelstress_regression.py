...
for dataset in datasets:
    ...
    # Load a dataset
    eeg_epochs = mne.io.read_epochs_eeglab(dataset).apply_baseline(
        baseline=(-1.2, -1.0)
    )
    ...
    y = trialinfo[
        [
            "trial_nr_total",  # Control for time on task
    ...
    y = np.concatenate(
        (
            y,
            np.multiply(
                trialinfo[["sequence_nr"]].to_numpy(),
                trialinfo[["last_feedback_scaled"]].to_numpy(),
            ),
        ),
        axis=1,
    )
